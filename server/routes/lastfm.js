...
router.get('/top', (req, res, next) => {
  axios.get(`http://ws.audioscrobbler.com/2.0/?method=chart.gettoptracks&api_key=${process.env.LASTFMKEY}&format=json&limit=10`)
  .then( response => {
...
router.get('/artist', (req, res, next) => {
  axios.get(`http://ws.audioscrobbler.com/2.0/?method=chart.gettopartists&api_key=${process.env.LASTFMKEY}&format=json&limit=10`)
  .then( response => {
    res.status(200).json(response.data);
  })
})
